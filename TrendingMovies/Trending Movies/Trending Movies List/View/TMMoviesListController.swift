...
extension TMMoviesListController: UITableViewDelegate, UITableViewDataSource, UITableViewDataSourcePrefetching {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: movieCellIdentifier, for: indexPath) as? TMMovieTableViewCell
    ...
    func tableView(_ tableView: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {
        ...
        for indexPath in indexPaths where indexPath.row < movies.count {
            let movie = movies[indexPath.row]
            SDWebImageManager.shared.loadImage(
                with: movie.posterURL(using: imagesConfig),
                options: [], progress: nil) {_, _, _, _, _, _ in }
        }
    }
